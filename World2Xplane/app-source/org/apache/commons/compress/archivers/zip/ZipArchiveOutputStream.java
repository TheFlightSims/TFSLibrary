...
/*      */   
/*  147 */   private final List<ZipArchiveEntry> entries = new LinkedList<ZipArchiveEntry>();
/*      */   
...
/*      */   
/*  173 */   private static final byte[] ZERO = new byte[] { 0, 0 };
/*      */   
/*  178 */   private static final byte[] LZERO = new byte[] { 0, 0, 0, 0 };
/*      */   
/*  183 */   private final Map<ZipArchiveEntry, Long> offsets = new HashMap<ZipArchiveEntry, Long>();
/*      */   
/*  193 */   private String encoding = "UTF8";
/*      */   
/*  201 */   private ZipEncoding zipEncoding = ZipEncodingHelper.getZipEncoding("UTF8");
/*      */   
...
/*  455 */     this.crc.reset();
/*  457 */     boolean actuallyNeedsZip64 = handleSizesAndCrc(bytesWritten, realCrc, effectiveMode);
...
/*  501 */         throw new ZipException("bad CRC checksum for entry " + this.entry.entry.getName() + ": " + Long.toHexString(this.entry.entry.getCrc()) + " instead of " + Long.toHexString(crc)); 
/*  508 */       if (this.entry.entry.getSize() != bytesWritten)
/*  509 */         throw new ZipException("bad size for entry " + this.entry.entry.getName() + ": " + this.entry.entry.getSize() + " instead of " + bytesWritten); 
/*      */     } else {
...
/*      */     } 
/*  521 */     boolean actuallyNeedsZip64 = (effectiveMode == Zip64Mode.Always || this.entry.entry.getSize() >= 4294967295L || this.entry.entry.getCompressedSize() >= 4294967295L);
/*  524 */     if (actuallyNeedsZip64 && effectiveMode == Zip64Mode.Never)
/*  525 */       throw new Zip64RequiredException(Zip64RequiredException.getEntryTooBigMessage(this.entry.entry)); 
...
/*  552 */       this.raf.seek(this.entry.localDataStart + 12L + 4L + getName(this.entry.entry).limit() + 4L);
...
/*  567 */         this.entry.entry.removeExtraField(Zip64ExtendedInformationExtraField.HEADER_ID);
...
/*  598 */     this.entry = new CurrentEntry((ZipArchiveEntry)archiveEntry);
...
/*  613 */       if (this.entry.entry.getMethod() == 0 && this.entry.entry.getSize() != -1L)
...
/*      */     } 
/*  666 */     if ((this.entry.entry.getSize() >= 4294967295L || this.entry.entry.getCompressedSize() >= 4294967295L) && effectiveMode == Zip64Mode.Never)
/*  669 */       throw new Zip64RequiredException(Zip64RequiredException.getEntryTooBigMessage(this.entry.entry)); 
/*      */   }
/*      */   
/*      */   private boolean shouldAddZip64Extra(ZipArchiveEntry entry, Zip64Mode mode) {
/*  689 */     return (mode == Zip64Mode.Always || entry.getSize() >= 4294967295L || entry.getCompressedSize() >= 4294967295L || (entry.getSize() == -1L && this.raf != null && mode != Zip64Mode.Never));
/*      */   }
/*      */   
...
/*  713 */     if (level < -1 || level > 9)
/*  715 */       throw new IllegalArgumentException("Invalid compression level: " + level); 
...
/*  781 */           this.def.setInput(b, offset + i * 8192, 8192);
...
/*  886 */     writeVersionNeededToExtractAndGeneralPurposeBits(zipMethod, (!encodable && this.fallbackToUTF8), hasZip64Extra(ze));
/*  890 */     this.written += 4L;
...
/*  957 */     if (this.createUnicodeExtraFields == UnicodeExtraFieldPolicy.ALWAYS || !encodable)
/*  959 */       ze.addExtraField(new UnicodePathExtraField(ze.getName(), name.array(), name.arrayOffset(), name.limit())); 
/*  965 */     String comm = ze.getComment();
/*  966 */     if (comm != null && !"".equals(comm)) {
/*  968 */       boolean commentEncodable = this.zipEncoding.canEncode(comm);
/*  970 */       if (this.createUnicodeExtraFields == UnicodeExtraFieldPolicy.ALWAYS || !commentEncodable) {
/*  972 */         ByteBuffer commentB = getEntryEncoding(ze).encode(comm);
/*  973 */         ze.addExtraField(new UnicodeCommentExtraField(comm, commentB.array(), commentB.arrayOffset(), commentB.limit()));
/*      */       } 
/*      */     } 
/*      */   }
/*      */   
...
/* 1018 */     boolean needsZip64Extra = (hasZip64Extra(ze) || ze.getCompressedSize() >= 4294967295L || ze.getSize() >= 4294967295L || lfhOffset >= 4294967295L);
...
/* 1035 */     writeOut(ZipShort.getBytes(ze.getPlatform() << 8 | (!this.hasUsedZip64 ? 20 : 45)));
/* 1038 */     this.written += 2L;
/* 1040 */     int zipMethod = ze.getMethod();
/* 1041 */     boolean encodable = this.zipEncoding.canEncode(ze.getName());
/* 1042 */     writeVersionNeededToExtractAndGeneralPurposeBits(zipMethod, (!encodable && this.fallbackToUTF8), needsZip64Extra);
...
/* 1060 */     if (ze.getCompressedSize() >= 4294967295L || ze.getSize() >= 4294967295L) {
...
/* 1130 */       if (ze.getCompressedSize() >= 4294967295L || ze.getSize() >= 4294967295L) {
...
/* 1162 */     if (numberOfEntries > 65535 && this.zip64Mode == Zip64Mode.Never)
...
/* 1172 */     byte[] num = ZipShort.getBytes(Math.min(numberOfEntries, 65535));
/* 1174 */     writeOut(num);
/* 1175 */     writeOut(num);
...
/*      */       return; 
/* 1200 */     if (!this.hasUsedZip64 && (this.cdOffset >= 4294967295L || this.cdLength >= 4294967295L || this.entries.size() >= 65535))
/* 1204 */       this.hasUsedZip64 = true; 
/* 1207 */     if (!this.hasUsedZip64)
/*      */       return; 
/* 1211 */     long offset = this.written;
/* 1213 */     writeOut(ZIP64_EOCD_SIG);
/* 1216 */     writeOut(ZipEightByteInteger.getBytes(44L));
...
/* 1349 */     Zip64ExtendedInformationExtraField z64 = (Zip64ExtendedInformationExtraField)ze.getExtraField(Zip64ExtendedInformationExtraField.HEADER_ID);
/* 1353 */     if (z64 == null)
...
/* 1376 */     return (ze.getExtraField(Zip64ExtendedInformationExtraField.HEADER_ID) != null);
/*      */   }
/*      */   
/*      */   private Zip64Mode getEffectiveZip64Mode(ZipArchiveEntry ze) {
/* 1389 */     if (this.zip64Mode != Zip64Mode.AsNeeded || this.raf != null || ze.getMethod() != 8 || ze.getSize() != -1L)
...
/* 1400 */     return (!encodable && this.fallbackToUTF8) ? ZipEncodingHelper.UTF8_ZIP_ENCODING : this.zipEncoding;
/*      */   }
/*      */   
...
/*      */     
/* 1441 */     public static final UnicodeExtraFieldPolicy NOT_ENCODEABLE = new UnicodeExtraFieldPolicy("not encodeable");
/*      */     
...
/* 1470 */       this.localDataStart = 0L;
/* 1474 */       this.dataStart = 0L;
/* 1479 */       this.bytesRead = 0L;
/* 1483 */       this.causedUseOfZip64 = false;
/*      */       this.entry = entry;
/*      */     }
/*      */   }
/*      */ }
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\org\apache\commons\compress\archivers\zip\ZipArchiveOutputStream.class
