  ...
  public void removeProhibitedAttrs() {
    ...
    for (byte b2 = 0; b2 < this.fAttrUseNum; b2++) {
      if ((this.fAttributeUses[b2]).fUse == 2)
        arrayOfXSAttributeUseImpl[this.fAttrUseNum - ++b1] = this.fAttributeUses[b2]; 
    } 
    byte b3 = 0;
    if (b1 > 0) {
      for (byte b = 0; b < this.fAttrUseNum; b++) {
        if ((this.fAttributeUses[b]).fUse != 2)
          for (byte b4 = 1;; b4++) {
            if (b4 > b1) {
              arrayOfXSAttributeUseImpl[b3++] = this.fAttributeUses[b];
              break;
            } 
            if ((this.fAttributeUses[b]).fAttrDecl.fName == (arrayOfXSAttributeUseImpl[this.fAttrUseNum - b1]).fAttrDecl.fName && (this.fAttributeUses[b]).fAttrDecl.fTargetNamespace == (arrayOfXSAttributeUseImpl[this.fAttrUseNum - b1]).fAttrDecl.fTargetNamespace)
              break; 
          }  
      } 
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\org\apache\xerces\impl\xs\XSAttributeGroupDecl.class
