...
class DatabaseManagerCommon {
  ...
  static void createTestTables(Statement paramStatement) {
    ...
    for (byte b = 0; b < arrayOfString.length; b++) {
      try {
        paramStatement.execute(arrayOfString[b]);
      } catch (SQLException sQLException) {}
    } 
  }
  
  static String createTestData(Statement paramStatement) throws SQLException {
    String[] arrayOfString1 = { 
        "White", "Karsen", "Smith", "Ringer", "May", "King", "Fuller", "Miller", "Ott", "Sommer", 
        "Schneider", "Steel", "Peterson", "Heiniger", "Clancy" };
    String[] arrayOfString2 = { 
        "Mary", "James", "Anne", "George", "Sylvia", "Robert", "Janet", "Michael", "Andrew", "Bill", 
        "Susanne", "Laura", "Bob", "Julia", "John" };
    String[] arrayOfString3 = { "Upland Pl.", "College Av.", "- 20th Ave.", "Seventh Av." };
    String[] arrayOfString4 = { 
        "New York", "Dallas", "Boston", "Chicago", "Seattle", "San Francisco", "Berne", "Oslo", "Paris", "Lyon", 
        "Palo Alto", "Olten" };
    String[] arrayOfString5 = { "Iron", "Ice Tea", "Clock", "Chair", "Telephone", "Shoe" };
    byte b1 = 50;
    byte b2;
    for (b2 = 0; b2 < b1; b2++) {
      paramStatement.execute("INSERT INTO Customer VALUES(" + b2 + ",'" + random(arrayOfString2) + "','" + random(arrayOfString1) + "','" + random(554) + " " + random(arrayOfString3) + "','" + random(arrayOfString4) + "')");
      paramStatement.execute("INSERT INTO Product VALUES(" + b2 + ",'" + random(arrayOfString5) + " " + random(arrayOfString5) + "'," + (20 + 2 * random(120)) + ")");
    } 
    for (b2 = 0; b2 < b1; b2++) {
      paramStatement.execute("INSERT INTO Invoice VALUES(" + b2 + "," + random(b1) + ",0.0)");
      for (int i = random(20) + 2; i >= 0; i--)
        paramStatement.execute("INSERT INTO Item VALUES(" + b2 + "," + i + "," + random(b1) + "," + (1 + random(24)) + ",1.5)"); 
    } 
  ...
  static void writeFile(String paramString1, String paramString2) {
    try {
      ...
      fileWriter.write(paramString2.toCharArray());
      fileWriter.close();
    } catch (IOException iOException) {
  ...
  static long testStatement(Statement paramStatement, String paramString, int paramInt) throws SQLException {
    ...
    while (b < paramInt) {
      String str;
      for (str = paramString;; str = str.substring(0, i) + (int)(Math.random() * b) + str.substring(i + 3)) {
        int i = str.indexOf("#r#");
        if (i == -1)
          break; 
      } 
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\org\hsqld\\util\DatabaseManagerCommon.class
