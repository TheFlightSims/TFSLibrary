...
class Token implements Serializable {
  ...
  final int getMinLength() {
    ...
    switch (this.type) {
      ...
      case 26:
        ...
        for (b2 = 1; b2 < size(); b2++) {
          int k = getChild(b2).getMinLength();
          if (k < j)
            j = k; 
        } 
        return j;
  ...
  final int getMaxLength() {
    ...
    switch (this.type) {
      case 1:
        i = 0;
        for (b1 = 0; b1 < size(); b1++) {
          int k = getChild(b1).getMaxLength();
          if (k < 0)
            return -1; 
          i += k;
        } 
      ...
      case 26:
        ...
        for (b2 = 1; j >= 0 && b2 < size(); b2++) {
          int k = getChild(b2).getMaxLength();
          if (k < 0) {
            j = -1;
            break;
          } 
  ...
  final int analyzeFirstCharacter(RangeToken paramRangeToken, int paramInt) {
    ...
    switch (this.type) {
      case 1:
        i = 0;
        for (b1 = 0; b1 < size() && (i = getChild(b1).analyzeFirstCharacter(paramRangeToken, paramInt)) == 0; b1++);
        return i;
      case 2:
        if (size() == 0)
          return 0; 
        ...
        bool = false;
        for (b2 = 0; b2 < size(); b2++) {
          j = getChild(b2).analyzeFirstCharacter(paramRangeToken, paramInt);
          if (j == 2)
            break; 
          if (j == 0)
            bool = true; 
  ...
  protected static RangeToken getRange(String paramString, boolean paramBoolean) {
    if (categories.size() == 0)
      synchronized (categories) {
        ...
        for (byte b1 = 0; b1 < arrayOfToken.length; b1++)
          arrayOfToken[b1] = createRange(); 
        for (byte b2 = 0; b2 < 65536; b2++) {
          int i = Character.getType((char)b2);
          if (i == 21 || i == 22) {
            if (b2 == '«' || b2 == '‘' || b2 == '‛' || b2 == '“' || b2 == '‟' || b2 == '‹')
              i = 29; 
            if (b2 == '»' || b2 == '’' || b2 == '”' || b2 == '›')
              i = 30; 
          } 
          arrayOfToken[i].addRange(b2, b2);
          switch (i) {
            case 1:
            case 2:
            case 3:
        ...
        for (byte b3 = 0; b3 < arrayOfToken.length; b3++) {
          if (categoryNames[b3] != null) {
            if (b3 == 0)
              arrayOfToken[b3].addRange(65536, 1114111); 
            categories.put(categoryNames[b3], arrayOfToken[b3]);
            categories2.put(categoryNames[b3], complementRanges(arrayOfToken[b3]));
          } 
        } 
        StringBuffer stringBuffer = new StringBuffer(50);
        for (byte b4 = 0; b4 < blockNames.length; b4++) {
          RangeToken rangeToken = createRange();
          if (b4 < 84) {
            int i = b4 * 2;
            char c1 = "\000ÿĀſƀɏɐʯʰ˿̀ͯͰϿЀӿ԰֏֐׿؀ۿ܀ݏހ޿ऀॿঀ৿਀੿઀૿଀୿஀௿ఀ౿ಀ೿ഀൿ඀෿฀๿຀໿ༀ࿿က႟Ⴀჿᄀᇿሀ፿Ꭰ᏿᐀ᙿ ᚟ᚠ᛿ក៿᠀᢯Ḁỿἀ῿ ⁯⁰₟₠⃏⃐⃿℀⅏⅐↏←⇿∀⋿⌀⏿␀␿⑀⑟①⓿─╿▀▟■◿☀⛿✀➿⠀⣿⺀⻿⼀⿟⿰⿿　〿぀ゟ゠ヿ㄀ㄯ㄰㆏㆐㆟ㆠㆿ㈀㋿㌀㏿㐀䶵一鿿ꀀ꒏꒐꓏가힣豈﫿ﬀﭏﭐ﷿︠︯︰﹏﹐﹯ﹰ﻾﻿﻿＀￯".charAt(i);
            char c2 = "\000ÿĀſƀɏɐʯʰ˿̀ͯͰϿЀӿ԰֏֐׿؀ۿ܀ݏހ޿ऀॿঀ৿਀੿઀૿଀୿஀௿ఀ౿ಀ೿ഀൿ඀෿฀๿຀໿ༀ࿿က႟Ⴀჿᄀᇿሀ፿Ꭰ᏿᐀ᙿ ᚟ᚠ᛿ក៿᠀᢯Ḁỿἀ῿ ⁯⁰₟₠⃏⃐⃿℀⅏⅐↏←⇿∀⋿⌀⏿␀␿⑀⑟①⓿─╿▀▟■◿☀⛿✀➿⠀⣿⺀⻿⼀⿟⿰⿿　〿぀ゟ゠ヿ㄀ㄯ㄰㆏㆐㆟ㆠㆿ㈀㋿㌀㏿㐀䶵一鿿ꀀ꒏꒐꓏가힣豈﫿ﬀﭏﭐ﷿︠︯︰﹏﹐﹯ﹰ﻾﻿﻿＀￯".charAt(i + 1);
            rangeToken.addRange(c1, c2);
          } else {
            int i = (b4 - 84) * 2;
            rangeToken.addRange(nonBMPBlockRanges[i], nonBMPBlockRanges[i + 1]);
          } 
          String str = blockNames[b4];
          ...
          setAlias(stringBuffer.toString(), str, true);
        } 
  ...
  protected static boolean isRegisterNonXS(String paramString) {
    return (nonxs == null) ? false : nonxs.containsKey(paramString);
  }
  ...
  static class UnionToken extends Token implements Serializable {
    ...
    void addChild(Token param1Token) {
      ...
      if (this.type == 2) {
        this.children.addElement(param1Token);
        return;
      } 
      if (param1Token.type == 1) {
        for (byte b1 = 0; b1 < param1Token.size(); b1++)
          addChild(param1Token.getChild(b1)); 
        return;
      } 
      ...
      Token token = this.children.elementAt(i - 1);
      if ((token.type != 0 && token.type != 10) || (param1Token.type != 0 && param1Token.type != 10)) {
        this.children.addElement(param1Token);
        return;
      } 
      byte b = (param1Token.type == 0) ? 2 : param1Token.getString().length();
      if (token.type == 0) {
        stringBuffer = new StringBuffer(2 + b);
        int j = token.getChar();
        if (j >= 65536) {
          stringBuffer.append(REUtil.decomposeToSurrogates(j));
        } else {
          stringBuffer.append((char)j);
        } 
        token = Token.createString(null);
        this.children.setElementAt(token, i - 1);
      } else {
        stringBuffer = new StringBuffer(token.getString().length() + b);
        stringBuffer.append(token.getString());
      } 
      if (param1Token.type == 0) {
        int j = param1Token.getChar();
        if (j >= 65536) {
          stringBuffer.append(REUtil.decomposeToSurrogates(j));
        } else {
          stringBuffer.append((char)j);
        ...
        stringBuffer.append(param1Token.getString());
      } 
      ((Token.StringToken)token).string = new String(stringBuffer);
    }
    ...
    Token getChild(int param1Int) {
      return this.children.elementAt(param1Int);
    }
    ...
    public String toString(int param1Int) {
      ...
      if (this.type == 1) {
        if (this.children.size() == 2) {
          ...
          if (token2.type == 3 && token2.getChild(0) == token1) {
            str = token1.toString(param1Int) + "+";
          } else if (token2.type == 9 && token2.getChild(0) == token1) {
            str = token1.toString(param1Int) + "+?";
          } else {
          ...
          for (byte b = 0; b < this.children.size(); b++)
            stringBuffer.append(((Token)this.children.elementAt(b)).toString(param1Int)); 
      ...
      if (this.children.size() == 2 && (getChild(1)).type == 7) {
        ...
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append(((Token)this.children.elementAt(0)).toString(param1Int));
  ...
  static class ModifierToken extends Token implements Serializable {
    ...
    public String toString(int param1Int) {
      return "(?" + ((this.add == 0) ? "" : REUtil.createOptionString(this.add)) + ((this.mask == 0) ? "" : REUtil.createOptionString(this.mask)) + ":" + this.child.toString(param1Int) + ")";
    }
  }
  ...
  static class ConcatToken extends Token implements Serializable {
    ...
    Token getChild(int param1Int) {
      return (param1Int == 0) ? this.child : this.child2;
    }
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\org\apache\xerces\impl\xpath\regex\Token.class
