...
public class BinaryData implements BlobData {
  ...
  public static final BinaryData singleBitOne = new BinaryData(new byte[] { Byte.MIN_VALUE }, 1L);
  ...
  public static BinaryData getBitData(byte[] paramArrayOfbyte, long paramLong) {
    return (paramLong == 1L) ? ((paramArrayOfbyte[0] == 0) ? singleBitZero : singleBitOne) : new BinaryData(paramArrayOfbyte, paramLong);
  }
  ...
  public long position(SessionInterface paramSessionInterface, byte[] paramArrayOfbyte, long paramLong) {
    return (paramArrayOfbyte.length > this.data.length) ? -1L : ((paramLong >= this.data.length) ? -1L : ArrayUtil.find(this.data, (int)paramLong, this.data.length, paramArrayOfbyte));
  }
  ...
  public boolean equals(Object paramObject) {
    return (paramObject instanceof BinaryData) ? ((Type.SQL_VARBINARY.compare(null, this, paramObject) == 0)) : false;
  }
  
  public int hashCode() {
    if (this.hashCode == 0) {
      int i = 0;
      for (byte b = 0; b < this.data.length && b < 32; b++)
        i = i * 31 + (0xFF & this.data[b]); 
      this.hashCode = i;
    } 
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\org\hsqldb\types\BinaryData.class
