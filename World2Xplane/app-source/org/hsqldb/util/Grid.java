...
class Grid extends Panel {
  ...
  public void setHead(String[] paramArrayOfString) {
    ...
    for (byte b = 0; b < this.iColCount; b++) {
      this.sColHead[b] = paramArrayOfString[b];
      this.iColWidth[b] = 100;
    } 
    this.iRowCount = 0;
    this.iRowHeight = 0;
  ...
  public void paint(Graphics paramGraphics) {
    ...
    if (this.iRowHeight == 0) {
      ...
      for (byte b1 = 0; b1 < this.iColCount; b1++)
        calcAutoWidth(b1); 
      adjustScroll();
    } 
    ...
    this.iFirstRow = b;
    for (j = this.iRowHeight + 1; j < this.iHeight && b < this.iRowCount; j += this.iRowHeight) {
      i = -this.iX;
      for (byte b1 = 0; b1 < this.iColCount; b1++) {
        int k = this.iColWidth[b1];
        Color color1 = Color.white;
        Color color2 = Color.black;
        this.gImage.setColor(color1);
        this.gImage.fillRect(i, j, k - 1, this.iRowHeight - 1);
        this.gImage.setColor(color2);
        this.gImage.drawString(getDisplay(b1, b), i + 2, j + this.iRowHeight - 5);
        this.gImage.setColor(Color.lightGray);
        this.gImage.drawLine(i + k - 1, j, i + k - 1, j + this.iRowHeight - 1);
        this.gImage.drawLine(i, j + this.iRowHeight - 1, i + k - 1, j + this.iRowHeight - 1);
        i += k;
      } 
      this.gImage.setColor(Color.white);
  ...
  public boolean mouseMove(Event paramEvent, int paramInt1, int paramInt2) {
    if (paramInt2 <= this.iRowHeight) {
      ...
      int j;
      for (j = this.iColCount - 1; j >= 0 && (paramInt1 <= -7 || paramInt1 >= 7); j--)
  ...
  private void calcAutoWidth(int paramInt) {
    ...
    for (byte b = 0; b < this.iRowCount; b++) {
      String[] arrayOfString = this.vData.elementAt(b);
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\org\hsqld\\util\Grid.class
