...
abstract class XSDAbstractParticleTraverser extends XSDAbstractTraverser {
  ...
  XSParticleDecl traverseAll(Element paramElement, XSDocumentInfo paramXSDocumentInfo, SchemaGrammar paramSchemaGrammar, int paramInt, XSObject paramXSObject) {
    ...
    xSModelGroupImpl.fAnnotations = xSObjectList;
    ...
    xSParticleDecl = checkOccurrences(xSParticleDecl, SchemaSymbols.ELT_ALL, (Element)paramElement.getParentNode(), paramInt, long_.longValue());
  ...
  private XSParticleDecl traverseSeqChoice(Element paramElement, XSDocumentInfo paramXSDocumentInfo, SchemaGrammar paramSchemaGrammar, int paramInt, boolean paramBoolean, XSObject paramXSObject) {
    ...
    xSParticleDecl.fValue = (XSTerm)xSModelGroupImpl;
    xSParticleDecl.fAnnotations = xSObjectList;
    xSParticleDecl = checkOccurrences(xSParticleDecl, paramBoolean ? SchemaSymbols.ELT_CHOICE : SchemaSymbols.ELT_SEQUENCE, (Element)paramElement.getParentNode(), paramInt, long_.longValue());
    this.fAttrChecker.returnAttrArray(arrayOfObject, paramXSDocumentInfo);
    return xSParticleDecl;
  }
  
  protected boolean hasAllContent(XSParticleDecl paramXSParticleDecl) {
    return (paramXSParticleDecl != null && paramXSParticleDecl.fType == 3) ? ((((XSModelGroupImpl)paramXSParticleDecl.fValue).fCompositor == 103)) : false;
  }
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\org\apache\xerces\impl\xs\traversers\XSDAbstractParticleTraverser.class
