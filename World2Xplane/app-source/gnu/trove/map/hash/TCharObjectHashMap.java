...
/*      */   
/*   60 */   private final TCharObjectProcedure<V> PUT_ALL_PROC = new TCharObjectProcedure<V>() {
/*      */       public boolean execute(char key, V value) {
/*   62 */         TCharObjectHashMap.this.put(key, value);
/*   63 */         return true;
/*      */       }
/*      */     };
/*      */   
...
/*   92 */     this.no_entry_key = Constants.DEFAULT_CHAR_NO_ENTRY_VALUE;
/*      */   }
/*      */   
/*      */   public TCharObjectHashMap(int initialCapacity, float loadFactor) {
/*  105 */     super(initialCapacity, loadFactor);
/*  106 */     this.no_entry_key = Constants.DEFAULT_CHAR_NO_ENTRY_VALUE;
/*      */   }
/*      */   
...
/*  153 */     char[] oldKeys = this._set;
/*  154 */     V[] oldVals = this._values;
/*  155 */     byte[] oldStates = this._states;
/*  157 */     this._set = new char[newCapacity];
...
/*  278 */     Set<? extends Map.Entry<? extends Character, ? extends V>> set = map.entrySet();
/*  279 */     for (Map.Entry<? extends Character, ? extends V> entry : set)
/*  280 */       put(((Character)entry.getKey()).charValue(), entry.getValue()); 
/*      */   }
/*      */   
...
/*      */   
/*      */   public char[] keys() {
/*  311 */     char[] keys = new char[size()];
/*  312 */     char[] k = this._set;
...
/*  328 */       dest = new char[this._size]; 
/*  331 */     char[] k = this._set;
...
/*  387 */     return new TCharObjectHashIterator<V>(this);
/*      */   }
/*      */   
...
/*  467 */     if (!(other instanceof TCharObjectMap))
/*  468 */       return false; 
/*  470 */     TCharObjectMap that = (TCharObjectMap)other;
/*  471 */     if (that.size() != size())
/*  472 */       return false; 
/*      */     try {
/*  475 */       TCharObjectIterator<V> iter = iterator();
/*  476 */       while (iter.hasNext()) {
/*  477 */         iter.advance();
/*  478 */         char key = iter.key();
...
/*  516 */       return TCharObjectHashMap.this.no_entry_key;
/*      */     }
/*      */     
/*      */     public int size() {
/*  521 */       return TCharObjectHashMap.this._size;
/*      */     }
/*      */     
/*      */     public boolean isEmpty() {
/*  526 */       return (TCharObjectHashMap.this._size == 0);
/*      */     }
/*      */     
/*      */     public boolean contains(char entry) {
/*  531 */       return TCharObjectHashMap.this.containsKey(entry);
/*      */     }
/*      */     
/*      */     public TCharIterator iterator() {
/*  536 */       return new TCharHashIterator(TCharObjectHashMap.this);
/*      */     }
/*      */     
/*      */     public char[] toArray() {
/*  541 */       return TCharObjectHashMap.this.keys();
/*      */     }
/*      */     
/*      */     public char[] toArray(char[] dest) {
/*  546 */       return TCharObjectHashMap.this.keys(dest);
/*      */     }
/*      */     
...
/*  556 */       return (null != TCharObjectHashMap.this.remove(entry));
/*      */     }
/*      */     
/*      */     public boolean containsAll(Collection<?> collection) {
/*  561 */       for (Object element : collection) {
/*  562 */         if (!TCharObjectHashMap.this.containsKey(((Character)element).charValue()))
/*  565 */           return false; 
/*      */       } 
/*  568 */       return true;
/*      */     }
...
/*  574 */         return true; 
/*  576 */       TCharIterator iter = collection.iterator();
/*  577 */       while (iter.hasNext()) {
/*  578 */         if (!TCharObjectHashMap.this.containsKey(iter.next()))
/*  579 */           return false; 
/*      */       } 
/*  582 */       return true;
/*      */     }
...
/*  587 */       for (char element : array) {
/*  588 */         if (!TCharObjectHashMap.this.containsKey(element))
/*  589 */           return false; 
/*      */       } 
/*  592 */       return true;
/*      */     }
...
/*  613 */       TCharIterator iter = iterator();
/*  614 */       while (iter.hasNext()) {
/*  616 */         if (!collection.contains(Character.valueOf(iter.next()))) {
/*  617 */           iter.remove();
/*  618 */           modified = true;
/*      */         } 
/*      */       } 
...
/*  630 */       TCharIterator iter = iterator();
...
/*  644 */       char[] set = TCharObjectHashMap.this._set;
/*  645 */       byte[] states = TCharObjectHashMap.this._states;
/*  647 */       for (int i = set.length; i-- > 0;) {
/*  648 */         if (states[i] == 1 && Arrays.binarySearch(array, set[i]) < 0) {
/*  649 */           TCharObjectHashMap.this.removeAt(i);
/*  650 */           changed = true;
/*      */         } 
/*      */       } 
/*  653 */       return changed;
...
/*  660 */         if (element instanceof Character) {
/*  661 */           char c = ((Character)element).charValue();
/*  662 */           if (remove(c))
/*  663 */             changed = true; 
/*      */         } 
/*      */       } 
...
/*  677 */       TCharIterator iter = collection.iterator();
/*  678 */       while (iter.hasNext()) {
/*  679 */         char element = iter.next();
...
/*      */     
/*      */     public void clear() {
/*  700 */       TCharObjectHashMap.this.clear();
/*      */     }
/*      */     
/*      */     public boolean forEach(TCharProcedure procedure) {
/*  705 */       return TCharObjectHashMap.this.forEachKey(procedure);
/*      */     }
/*      */     
/*      */     public boolean equals(Object other) {
/*  710 */       if (!(other instanceof TCharSet))
/*  711 */         return false; 
/*  713 */       TCharSet that = (TCharSet)other;
/*  714 */       if (that.size() != size())
/*  715 */         return false; 
/*  717 */       for (int i = TCharObjectHashMap.this._states.length; i-- > 0;) {
/*  718 */         if (TCharObjectHashMap.this._states[i] == 1 && 
/*  719 */           !that.contains(TCharObjectHashMap.this._set[i]))
/*  720 */           return false; 
/*      */       } 
/*  724 */       return true;
/*      */     }
...
/*      */     public int hashCode() {
/*  729 */       int hashcode = 0;
/*  730 */       for (int i = TCharObjectHashMap.this._states.length; i-- > 0;) {
/*  731 */         if (TCharObjectHashMap.this._states[i] == 1)
/*  732 */           hashcode += HashFunctions.hash(TCharObjectHashMap.this._set[i]); 
/*      */       } 
/*  735 */       return hashcode;
/*      */     }
/*      */     
...
/*  742 */       for (int i = TCharObjectHashMap.this._states.length; i-- > 0;) {
/*  743 */         if (TCharObjectHashMap.this._states[i] == 1) {
/*  744 */           if (first) {
/*  744 */             first = false;
/*      */           } else {
/*  745 */             buf.append(",");
/*      */           } 
/*  746 */           buf.append(TCharObjectHashMap.this._set[i]);
/*      */         } 
/*      */       } 
...
/*  778 */       return new TCharObjectValueHashIterator(TCharObjectHashMap.this) {
/*      */           protected V objectAtIndex(int index) {
/*  780 */             return TCharObjectHashMap.this._values[index];
/*      */           }
/*      */         };
/*      */     }
/*      */     
/*      */     public boolean containsElement(V value) {
/*  786 */       return TCharObjectHashMap.this.containsValue(value);
/*      */     }
/*      */     
/*      */     public boolean removeElement(V value) {
/*  790 */       V[] values = TCharObjectHashMap.this._values;
/*  791 */       byte[] states = TCharObjectHashMap.this._states;
/*  793 */       for (int i = values.length; i-- > 0;) {
/*  794 */         if (states[i] == 1 && (
/*  795 */           value == values[i] || (null != values[i] && values[i].equals(value)))) {
/*  797 */           TCharObjectHashMap.this.removeAt(i);
/*  798 */           return true;
/*      */         } 
/*      */       } 
/*  802 */       return false;
...
/*      */       
/*      */       protected V objectAtIndex(int index) {
/*  817 */         byte[] states = TCharObjectHashMap.this._states;
...
/*      */     
/*      */     public void clear() {
/*  855 */       TCharObjectHashMap.this.clear();
/*      */     }
/*      */     
...
/*      */     
/*      */     public int size() {
/*  863 */       return TCharObjectHashMap.this.size();
/*      */     }
/*      */     
...
/*      */     
/*      */     public boolean isEmpty() {
/*  897 */       return TCharObjectHashMap.this.isEmpty();
/*      */     }
/*      */     
...
/*  993 */     this.no_entry_key = in.readChar();
/*  996 */     int size = in.readInt();
/*  997 */     setUp(size);
/* 1000 */     while (size-- > 0) {
/* 1001 */       char key = in.readChar();
...
/* 1010 */     forEachEntry(new TCharObjectProcedure<V>() {
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\gnu\trove\map\hash\TCharObjectHashMap.class
