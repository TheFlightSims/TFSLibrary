...
/*      */   
/*   60 */   private final TIntObjectProcedure<V> PUT_ALL_PROC = new TIntObjectProcedure<V>() {
/*      */       public boolean execute(int key, V value) {
/*   62 */         TIntObjectHashMap.this.put(key, value);
/*   63 */         return true;
/*      */       }
/*      */     };
/*      */   
...
/*   92 */     this.no_entry_key = Constants.DEFAULT_INT_NO_ENTRY_VALUE;
/*      */   }
/*      */   
/*      */   public TIntObjectHashMap(int initialCapacity, float loadFactor) {
/*  105 */     super(initialCapacity, loadFactor);
/*  106 */     this.no_entry_key = Constants.DEFAULT_INT_NO_ENTRY_VALUE;
/*      */   }
/*      */   
...
/*  153 */     int[] oldKeys = this._set;
/*  154 */     V[] oldVals = this._values;
/*  155 */     byte[] oldStates = this._states;
/*  157 */     this._set = new int[newCapacity];
...
/*  278 */     Set<? extends Map.Entry<? extends Integer, ? extends V>> set = map.entrySet();
/*  279 */     for (Map.Entry<? extends Integer, ? extends V> entry : set)
/*  280 */       put(((Integer)entry.getKey()).intValue(), entry.getValue()); 
/*      */   }
/*      */   
...
/*  387 */     return new TIntObjectHashIterator<V>(this);
/*      */   }
/*      */   
...
/*  467 */     if (!(other instanceof TIntObjectMap))
/*  468 */       return false; 
/*  470 */     TIntObjectMap that = (TIntObjectMap)other;
/*  471 */     if (that.size() != size())
/*  472 */       return false; 
/*      */     try {
/*  475 */       TIntObjectIterator<V> iter = iterator();
/*  476 */       while (iter.hasNext()) {
/*  477 */         iter.advance();
/*  478 */         int key = iter.key();
...
/*  516 */       return TIntObjectHashMap.this.no_entry_key;
/*      */     }
/*      */     
/*      */     public int size() {
/*  521 */       return TIntObjectHashMap.this._size;
/*      */     }
/*      */     
/*      */     public boolean isEmpty() {
/*  526 */       return (TIntObjectHashMap.this._size == 0);
/*      */     }
/*      */     
/*      */     public boolean contains(int entry) {
/*  531 */       return TIntObjectHashMap.this.containsKey(entry);
/*      */     }
/*      */     
/*      */     public TIntIterator iterator() {
/*  536 */       return new TIntHashIterator(TIntObjectHashMap.this);
/*      */     }
/*      */     
/*      */     public int[] toArray() {
/*  541 */       return TIntObjectHashMap.this.keys();
/*      */     }
/*      */     
/*      */     public int[] toArray(int[] dest) {
/*  546 */       return TIntObjectHashMap.this.keys(dest);
/*      */     }
/*      */     
...
/*  556 */       return (null != TIntObjectHashMap.this.remove(entry));
/*      */     }
/*      */     
/*      */     public boolean containsAll(Collection<?> collection) {
/*  561 */       for (Object element : collection) {
/*  562 */         if (!TIntObjectHashMap.this.containsKey(((Integer)element).intValue()))
/*  565 */           return false; 
/*      */       } 
/*  568 */       return true;
/*      */     }
...
/*  574 */         return true; 
/*  576 */       TIntIterator iter = collection.iterator();
/*  577 */       while (iter.hasNext()) {
/*  578 */         if (!TIntObjectHashMap.this.containsKey(iter.next()))
/*  579 */           return false; 
/*      */       } 
/*  582 */       return true;
/*      */     }
...
/*  587 */       for (int element : array) {
/*  588 */         if (!TIntObjectHashMap.this.containsKey(element))
/*  589 */           return false; 
/*      */       } 
/*  592 */       return true;
/*      */     }
...
/*  613 */       TIntIterator iter = iterator();
/*  614 */       while (iter.hasNext()) {
/*  616 */         if (!collection.contains(Integer.valueOf(iter.next()))) {
/*  617 */           iter.remove();
/*  618 */           modified = true;
/*      */         } 
/*      */       } 
...
/*  630 */       TIntIterator iter = iterator();
...
/*  644 */       int[] set = TIntObjectHashMap.this._set;
/*  645 */       byte[] states = TIntObjectHashMap.this._states;
/*  647 */       for (int i = set.length; i-- > 0;) {
/*  648 */         if (states[i] == 1 && Arrays.binarySearch(array, set[i]) < 0) {
/*  649 */           TIntObjectHashMap.this.removeAt(i);
/*  650 */           changed = true;
/*      */         } 
/*      */       } 
/*  653 */       return changed;
...
/*  660 */         if (element instanceof Integer) {
/*  661 */           int c = ((Integer)element).intValue();
/*  662 */           if (remove(c))
/*  663 */             changed = true; 
/*      */         } 
/*      */       } 
...
/*  677 */       TIntIterator iter = collection.iterator();
/*  678 */       while (iter.hasNext()) {
/*  679 */         int element = iter.next();
...
/*      */     
/*      */     public void clear() {
/*  700 */       TIntObjectHashMap.this.clear();
/*      */     }
/*      */     
/*      */     public boolean forEach(TIntProcedure procedure) {
/*  705 */       return TIntObjectHashMap.this.forEachKey(procedure);
/*      */     }
/*      */     
/*      */     public boolean equals(Object other) {
/*  710 */       if (!(other instanceof TIntSet))
/*  711 */         return false; 
/*  713 */       TIntSet that = (TIntSet)other;
/*  714 */       if (that.size() != size())
/*  715 */         return false; 
/*  717 */       for (int i = TIntObjectHashMap.this._states.length; i-- > 0;) {
/*  718 */         if (TIntObjectHashMap.this._states[i] == 1 && 
/*  719 */           !that.contains(TIntObjectHashMap.this._set[i]))
/*  720 */           return false; 
/*      */       } 
/*  724 */       return true;
/*      */     }
...
/*      */     public int hashCode() {
/*  729 */       int hashcode = 0;
/*  730 */       for (int i = TIntObjectHashMap.this._states.length; i-- > 0;) {
/*  731 */         if (TIntObjectHashMap.this._states[i] == 1)
/*  732 */           hashcode += HashFunctions.hash(TIntObjectHashMap.this._set[i]); 
/*      */       } 
/*  735 */       return hashcode;
/*      */     }
/*      */     
...
/*  742 */       for (int i = TIntObjectHashMap.this._states.length; i-- > 0;) {
/*  743 */         if (TIntObjectHashMap.this._states[i] == 1) {
/*  744 */           if (first) {
/*  744 */             first = false;
/*      */           } else {
/*  745 */             buf.append(",");
/*      */           } 
/*  746 */           buf.append(TIntObjectHashMap.this._set[i]);
/*      */         } 
/*      */       } 
...
/*  778 */       return new TIntObjectValueHashIterator(TIntObjectHashMap.this) {
/*      */           protected V objectAtIndex(int index) {
/*  780 */             return TIntObjectHashMap.this._values[index];
/*      */           }
/*      */         };
/*      */     }
/*      */     
/*      */     public boolean containsElement(V value) {
/*  786 */       return TIntObjectHashMap.this.containsValue(value);
/*      */     }
/*      */     
/*      */     public boolean removeElement(V value) {
/*  790 */       V[] values = TIntObjectHashMap.this._values;
/*  791 */       byte[] states = TIntObjectHashMap.this._states;
/*  793 */       for (int i = values.length; i-- > 0;) {
/*  794 */         if (states[i] == 1 && (
/*  795 */           value == values[i] || (null != values[i] && values[i].equals(value)))) {
/*  797 */           TIntObjectHashMap.this.removeAt(i);
/*  798 */           return true;
/*      */         } 
/*      */       } 
/*  802 */       return false;
...
/*      */       
/*      */       protected V objectAtIndex(int index) {
/*  817 */         byte[] states = TIntObjectHashMap.this._states;
...
/*      */     
/*      */     public void clear() {
/*  855 */       TIntObjectHashMap.this.clear();
/*      */     }
/*      */     
...
/*      */     
/*      */     public int size() {
/*  863 */       return TIntObjectHashMap.this.size();
/*      */     }
/*      */     
...
/*      */     
/*      */     public boolean isEmpty() {
/*  897 */       return TIntObjectHashMap.this.isEmpty();
/*      */     }
/*      */     
...
/*  993 */     this.no_entry_key = in.readInt();
/*  996 */     int size = in.readInt();
/*  997 */     setUp(size);
/* 1000 */     while (size-- > 0) {
/* 1001 */       int key = in.readInt();
...
/* 1010 */     forEachEntry(new TIntObjectProcedure<V>() {
...
/* Location:              D:\#source-code\TheFlightSims\TFSLibrary\World2Xplane\World2XPlane.jar!\gnu\trove\map\hash\TIntObjectHashMap.class
